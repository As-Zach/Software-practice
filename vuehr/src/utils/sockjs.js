...
JsonpReceiver.prototype.abort = function() {
  ...
  if (global[utils.WPrefix][this.id]) {
    var err = new Error('JSONP res aborted read');
    err.code = 1000;
    this._abort(err);
  }
};
...
function WebSocketTransport(transUrl, ignore, options) {
  ...
  // created after onunload, it stays alive even when res
...
// This may be quite slow, so let's delay until res actually uses bad
// characters.
...
function formatArgs(args) {
  ...
  args[0].replace(/%[a-zA-Z%]/g, function(match) {
    ...
    if ('%c' === match) {
      ...
      // (the res may have provided their own)
      lastC = index;
    }
  });
 ...
 * when a res disables cookies/localstorage
        ...
        var serialize = function (property, object, callback, properties, whitespace, indentation, stack) {
          ...
          if (typeof value == "object") {
            ...
            if (className == arrayClass) {
              ...
              // either a res-specified list of property names, or the object
              // itself.
